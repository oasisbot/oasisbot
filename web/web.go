		...
		r.Group(func(r chi.Router) {
			...
			OUTER:
				for _, guild := range *guilds {
					...
					} else if data, _ := guilds_cache.GuildData.Fetch(guild.ID, false); data != nil {
						if user == nil {
							user, err = api.GetMe(r)
							if err != nil {
								continue OUTER
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/api/guild_data", func(w http.ResponseWriter, r *http.Request) {
					...
					g.Go(func() error {
						result, err := guilds_cache.GuildData.Fetch(id, true)
						if result != nil {
				...
				r.Patch("/api/dashboard/settings", func(w http.ResponseWriter, r *http.Request) {
					...
					currentData, _ := guilds_cache.GuildData.Fetch(guild.ID, true)
					if !hasHighAuthority {
					...
					if err := guilds_cache.GuildData.Update(updated); err == nil {
						var roles []discordgo.Role
						var channels []discordgo.Channel
						for _, role := range guild.Roles {
							roles = append(roles, *role)
