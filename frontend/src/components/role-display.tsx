...
import { makeStyles, createStyles, Theme, Chip, IconButton } from '@material-ui/core'
...
export default function RoleDisplay ({ allRoles, roles, variant = undefined, onRolesUpdate }: RoleDisplayProps) {
    const classes = useStyles()
    const [roleSelectOpen, setRoleSelectOpen] = React.useState(false)
    const [roleSelectAnchor, setRoleSelectAnchor] = React.useState<HTMLElement | undefined>()
    ...
    return (
        <div className={classes.roleDisplay}>
            { variant == 'add' ?
            <IconButton style={{color: '#dddddd', marginTop: '0px'}} size='small' onClick={addClicked}>
                <AddCircleIcon />
            </IconButton>
            : null}
            {roles.map(role => {
                return <div key={role.id} className={classes.roleItem}>
                    <Chip 
                        ...
                        <div style={{
                            ...
                            <svg style={{width: '15px', height: '15px'}}>
                                <circle cx='7.5' cy='7.5' r='7.5' fill={
                                    ...
                                    typeof(role.color) == 'number' ? `#${role.color.toString(16)}` : role.color
                                    : '#757575ff'
                                }>
                                </circle>
                            </svg>
                            ...
                            className={`${classes.clearIcon} ${!LightOrDarkColorNum(role.color) ? classes.dark : ''}`} 
                            color='inherit'/>
                        </div>
                    } 
                    variant='outlined' 
                    ...
                    paddingLeft: '3px',
                    borderColor: typeof(role.color) == 'number' ? `#${role.color.toString(16)}` : role.color}}
                    label={role.name}/>
                </div>
            })} 
            {variant == 'disabled' ?
            ...
            : variant == 'add' ?
            <RoleSelect 
            roles={allRoles ? allRoles.filter(x => roles.find(y => y.id == x.id) == undefined) : []} 
            open={roleSelectOpen} 
