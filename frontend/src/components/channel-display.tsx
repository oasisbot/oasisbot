...
export default function ChannelDisplay ({ channels, allChannels = [], onChannelsUpdate }: ChannelDisplayProps) {
    const classes = useStyles()
    const [channelSelectOpen, setChannelSelectOpen] = React.useState(false)
    const [channelSelectAnchor, setChannelSelectAnchor] = React.useState<HTMLElement | undefined>()
        ...
        <div className={classes.channelDisplay}>
            <div style={{height: '50px', display: 'inline-block'}}>
                <div style={{display: 'flex', alignItems: 'center', height: '100%'}}>
                    <IconButton style={{color: '#dddddd'}} size='small' onClick={addClicked}>
                        <AddCircleIcon />
                    </IconButton>
                </div>
            </div>
            {channels.map(c => {
                ...
                <div className={classes.channelObject}>
                    <div className={classes.channelObjectContent}>
                        <img alt='#' src={HashIcon} style={{marginRight: '5px'}}/>
            ...
            <ChannelSelect 
            channels={allChannels ? allChannels.filter(x => channels.find(y => y.id === x.id) === undefined) : []} 
