...
export default function CommandViewRole ({ command, editing, onClose, onSave }: CommandViewRoleProps) {
    ...
    const [assignedRoles, setAssignedRoles] = React.useState(command.AssignedRoles ? guild?.Roles.filter(x => command.AssignedRoles.includes(x.id)) || [] : [])
    ...
    return (
        <div className={classes.root}>
            ...
            <h4 style={{color: '#bbbbbb', marginBottom: '10px'}}>BOT GIVES ROLES:</h4>
            <RoleDisplay 
            variant='add' 
            roles={assignedRoles} 
            onRolesUpdate={(roles) => setAssignedRoles(roles)}
            allRoles={guild?.Roles.filter(x => !assignedRoles.includes(x))} />
            <CommonSettings
