...
import { makeStyles, createStyles, Theme, Button, Divider } from '@material-ui/core'
...
export default function CommandViewRandomized ({ command, editing, onClose, onSave }: CommandViewRandomizedProps) {
    ...
    const [responseValues, setResponseValues] = React.useState(command.Responses)
    ...
    return (
        <div className={classes.root}>
            ...
            <h4 style={{color: '#bbbbbb', marginBottom: '10px'}}>RESPONSES:</h4>
            {
                        ...
                        <>
                            ...
                            <div>
                                <div style={{display: 'flex', flexFlow: 'row', alignItems: 'center'}}>
                                    <h4 style={{color: '#bbbbbb'}}>{`Response ${index + 1}`}</h4>
                                    <div style={{margin: '0 10px'}}><Button onClick={() => {
                                        const newResponses = responses.filter((_, i) => i != index)
                                        const newResponseValues = responseValues.filter((_, i) => i != index)
                            ...
                            <TextAreaCounter 
                            variant='expandable'
                            textAreaProps={{value: responseValues[index], onChange: (value: string) => {
